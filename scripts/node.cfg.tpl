...
-   content: |
        [Service]
        Environment="KUBELET_EXTRA_ARGS=--cgroup-driver=systemd --cloud-provider=openstack --cloud-config=/etc/kubernetes/cloud-config --node-labels=node-role.kubernetes.io/node= --serialize-image-pulls=false --feature-gates=ReadOnlyAPIDataVolumes=false --housekeeping-interval 1s"
